...
namespace SpajamHonsen.Controllers
{
    public class TweetSummyController : ApiController
    {
        ...
        public ExpressInformation GetTweetSummy(string id)
        {
            ...
            {
                express1 = db.HVCLog.Where(item => item.SpotID == id && item.Expression == 1 && item.TweetID != null).Sum(item => item.Expression);
            }
            var express2 = 0;
            if (db.HVCLog.Any(item => item.SpotID == id && item.Expression == 2 && item.TweetID != null))
            {
                express2 = db.HVCLog.Where(item => item.SpotID == id && item.Expression == 2 && item.TweetID != null).Sum(item => item.Expression);
            }
            var express3 = 0;
            if (db.HVCLog.Any(item => item.SpotID == id && item.Expression == 3 && item.TweetID != null))
            {
                express3 = db.HVCLog.Where(item => item.SpotID == id && item.Expression == 3 && item.TweetID != null).Sum(item => item.Expression);
            }
            var express4 = 0;
            if (db.HVCLog.Any(item => item.SpotID == id && item.Expression == 4 && item.TweetID != null))
            {
                express4 = db.HVCLog.Where(item => item.SpotID == id && item.Expression == 4 && item.TweetID != null).Sum(item => item.Expression);
            }
            var express5 = 0;
            if (db.HVCLog.Any(item => item.SpotID == id && item.Expression == 5 && item.TweetID != null))
            {
                express5 = db.HVCLog.Where(item => item.SpotID == id && item.Expression == 5 && item.TweetID != null).Sum(item => item.Expression);
            }
